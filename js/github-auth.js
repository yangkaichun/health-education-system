...
// 預設的 GitHub Token - 直接內建在代碼中
//const DEFAULT_TOKEN = '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';
let DEFAULT_TOKEN = null; // 預設為 null

async function fetchDefaultToken() {
    try {
        const response = await fetch('/api/github-token'); // 你需要自己在後端提供這個安全 API
        if (response.ok) {
            const data = await response.json();
            DEFAULT_TOKEN = data.token;
            console.log('Successfully fetched default token.');
        } else {
            console.error('Failed to fetch default token, status:', response.status);
        }
    } catch (error) {
...
// 修改 initializeAuth 加上等待 fetchDefaultToken
async function initializeAuth() {
    console.log('Initializing GitHub authentication...');

    await fetchDefaultToken(); // 確保先取得 Token
    ...
    if (!token) {
        ...
        if (!DEFAULT_TOKEN) {
            console.error('Default token is unavailable');
            showError('預設 GitHub Token 無法取得，請聯繫系統管理員。');
            showAuthForm();
            return;
        }
        token = DEFAULT_TOKEN;
