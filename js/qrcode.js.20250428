            ...
            .then(function(cameras) {
                if (cameras.length > 0) {
                    ...
                    if (isMobile) {
                        ...
                        if (backCameraFoundIndex !== -1) {
                            // 如果找到非前置鏡頭，使用它
                            selectedCameraIndex = backCameraFoundIndex;
                            console.log('找到非前置鏡頭，使用索引:', selectedCameraIndex);
                        } else {
                            // 如果沒有找到非前置鏡頭 (可能只有前置鏡頭)，則使用第一個相機 (預設值 0)
                            console.log('未找到非前置鏡頭，使用第一個相機 (索引 0)');
                        }

                    } else {
                         // 如果不是行動裝置，沿用原本的後置優先邏輯 (可能用於桌面有外接相機的情況)
                         ...
                         console.log('非行動裝置，使用原本的後置優先邏輯，選擇索引:', selectedCameraIndex);
                    }
